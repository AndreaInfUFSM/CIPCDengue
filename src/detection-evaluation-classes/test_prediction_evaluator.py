...
def test_inspect_single_prediction(entry_name, str_framework, dataset_class, detector_class, dataset_path, model_path, configs):

    print(f'\nEvaluating predictions for a single image using {str_framework}. This may take a little long to initialize...')

    dataset = dataset_class(dataset_path)
    detector = detector_class(model_path, configs)
...
def test_evaluate_prediction(str_framework, dataset_class, detector_class, dataset_path, model_path, configs):
    ...
    for class_id in dataset.get_unique_classes():
        ...
        filtered_pred_bboxes, filtered_pred_labels = filter_dataset_predictions(class_id, configs['score_threshold'], predicted_bboxes, predicted_labels, predicted_scores)
        
        # print(filtered_truth_bboxes)
        # print(filtered_truth_labels)
        # print(filtered_pred_bboxes)

        confusion_matrix_for_class = PredictionEvaluator.evaluate(filtered_truth_bboxes, filtered_pred_bboxes, configs['iou_threshold'])
...
def main():
    ...
    test_inspect_single_prediction('0', 'Tensorflow', TFDataset, TFObjectDetector, tf_test_dataset_path, tf_saved_model_path, configs)
    test_evaluate_prediction('Tensorflow', TFDataset, TFObjectDetector, tf_test_dataset_path, tf_saved_model_path, configs)
    ...
    test_inspect_single_prediction('000000000379.jpg', 'Yolo', YoloDataset, YoloObjectDetector, yolo_test_dataset_path, yolo_model_pt, configs)
    test_evaluate_prediction('Yolo', YoloDataset, YoloObjectDetector, yolo_test_dataset_path, yolo_model_pt, configs)
